...
function getList() {
    ...
    axios(config)
        ...
        `;
        let stationListId = document.getElementById('station-list');
        stationListId.innerHTML = html;
        // Filter stations by name
        const filterInput = document.getElementById("filter");
        const stationMainList = document.getElementById("station-main-list");
        const list = stationMainList.getElementsByClassName("stations-list");
        filterInput.addEventListener("input", () => {
            ...
            let station = "";
            let stationsCount = document.getElementById('stations-count');
            flexStationsCount === 1
                ? station = "station"
                : station = "stations";
            flexStationsCount === 0
...
$(document).ready(function ($) {
    ...
    radioAdd.onclick = function () {
        ...
        let radioURL = document.getElementById("radio-url");
        let minLength = 16;
        let nameMinLength = 3;
    ...
    radioDelete.onclick = async () => {
        let radioId = document.getElementById("radio-id");
    ...
    stationUpdate.onclick = async () => {
        let radioIdUpdate = document.getElementById("radio-id-update");
        console.log("Radio id:", radioIdUpdate.value);
        let radioName = document.getElementById("radio-update-name");
        console.log("Radio name:", radioName.value);
        let radioURL = document.getElementById("radio-update-url");
    ...
    getStationId.onclick = async function () {
        ...
        const res = await axios.get(`${baseUrl}/api/stations/${stationName.value}`);
        console.log(JSON.stringify(res.data));
        let data = res.data;
        res.status === 200 ? console.log(res.data) : console.log("Error");
        console.log(data.id);
        let stationById = document.getElementById('station-by-id');
